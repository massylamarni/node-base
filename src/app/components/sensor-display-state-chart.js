...
import { checkStruct, setChart, getChartData, getMissingData, updateChart, fetchRawDataArray, getGroupedChartData } from '../functions/main';
...
export default function SensorDisplayStateChart({ chartInfo }) {
  ...
  useEffect(() => {
    if (checkStruct(data)) {
      ...
      setChartData({
        "missingDataBefore": getChartData(missingData.before),
        "sensorData": getGroupedChartData(getChartData(data), chartTimeRange),
        "missingDataAfter": getChartData(missingData.after)
      });
