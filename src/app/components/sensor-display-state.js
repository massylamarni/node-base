...
import { checkStruct, getDisplayStateData, setChart, getChartData, getMissingData, updateChart, fetchRawDataArray } from '../functions/main';
...
export default function SensorDisplayState({ chartInfo }) {
  ...
  let stateData = getDisplayStateData(data, getChartTimeRange());
  ...
  useEffect(() => {
    if (checkStruct(data)) {
      ...
      stateData = getDisplayStateData(data, chartTimeRange);
      setChartData({
        "missingDataBefore": getChartData(missingData.before),
  ...
  return (
    <div className="sensor-display">
      <div className="sensor-display-head">
        <div>
            {!stateData.state && <svg width="800px" height="800px" viewBox="0 0 24 24" fill="none">
                <path 
        ...
        <div>
          <div className="sensor-display-title">
            <div>{(stateData.state === '-1') ? 'Unknown' : (stateData.state ? chartInfo.info.true : chartInfo.info.false)}</div>
            <div></div>
          </div>
      ...
      <div className="sensor-display-body">
        <div className="sensor-display-peaks">
          <div>
            ...
            <div>{(typeof stateData.value === 'boolean') ? (stateData.state ? chartInfo.info.true : chartInfo.info.false) : stateData.value}</div>
          </div>
        </div>
